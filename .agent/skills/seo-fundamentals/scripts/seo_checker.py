...
def check_page(file_path: Path) -> dict:
    ...
    # 1. Title tag (and Next.js metadata)
    has_title = any(x in content for x in ['<title', 'title=', 'Head>', 'metadata =', 'title: "'])
    if not has_title and is_layout:
        issues.append("Missing <title> tag")
    
    # 2. Meta description (and Next.js metadata)
    has_description = any(x in content.lower() for x in ['name="description"', 'description:', 'description="'])
    if not has_description and is_layout:
        issues.append("Missing meta description")
    
    # 3. Open Graph tags (and Next.js metadata)
    has_og = any(x in content.lower() for x in ['og:', 'property="og:', 'opengraph:'])
    if not has_og and is_layout:
        issues.append("Missing Open Graph tags")
    
    # 4. Heading hierarchy - multiple H1s (ignore commented out ones)
    safe_content = re.sub(r'{\/\*.*?\*\/}', '', content, flags=re.DOTALL) # JSX comments
    safe_content = re.sub(r'<!--.*?-->', '', safe_content, flags=re.DOTALL) # HTML comments
    h1_matches = re.findall(r'<h1[^>]*>', safe_content, re.I)
    if len(h1_matches) > 1:
